...
class FolderToolsTab(BaseTab):
    ...
    def load_config(self):
        """从配置文件加载设置"""
        config = self.config.get('folder_tools')
        if config:
            # 加载目标文件夹
    ...
    def save_config(self):
        ...
        self.config.set('folder_tools', 'target_folder', target_folder)
    ...
    def init_ui(self):
        ...
        self.target_entry = HistoryEntry(
            self.frame, self.config, 'folder_tools', 'target_folder', label_text="目标文件夹"
        )
        ...
        # 设置日志系统
        log_file = os.path.join(self.log_dir, 'folder_tools.log')
        self.logger = setup_logger('folder_tools', self.log_text, log_file)
    ...
    def delete_symlinks(self):
        target_folder = self.config.get('folder_tools', 'target_folder')
