...
class SymlinkExportTab(BaseTab):
    ...
    def load_config(self):
        """从配置文件加载设置"""
        config = self.config.get('symlink_export')
        if config:
            # 加载线程数
    ...
    def save_config(self):
        ...
        self.config.set('symlink_export', 'target_folder', target_folder)
        try:
        ...
        self.config.set('symlink_export', 'thread_count', thread_count)
        self.config.set('symlink_export', 'link_suffixes', link_suffixes)
        self.config.set('symlink_export', 'meta_suffixes', meta_suffixes)
        self.config.set('symlink_export', 'only_tvshow_nfo', bool(self.only_tvshow_nfo_var.get()))
        self.config.set('symlink_export', 'enable_replace_path', bool(self.replace_path_var.get()))
        self.config.set('symlink_export', 'original_path', self.original_path_entry.get().strip())
        self.config.set('symlink_export', 'replace_path', self.replace_path_entry.get().strip())
    ...
    def init_ui(self):
        ...
        self.target_entry = HistoryEntry(folder_frame, self.config, 'symlink_export', 'target_folder')
        ...
        self.original_path_entry = HistoryEntry(
            left_container, self.config, 'symlink_export', 'original_path', label_text=None
        )
        ...
        self.replace_path_entry = HistoryEntry(
            right_container, self.config, 'symlink_export', 'replace_path', label_text=None
        )
        ...
        self.logger = setup_logger('symlink_export', self.log_text, log_file)
    ...
    def validate_sync_config(self):
        ...
        allowed_extensions = tuple(self.config.get('symlink_export', 'meta_suffixes'))
        enable_replace_path = self.config.get('symlink_export', 'enable_replace_path')
        original_path = self.config.get('symlink_export', 'original_path')
        replace_path = self.config.get('symlink_export', 'replace_path')
        ...
        only_tvshow_nfo = self.config.get('symlink_export', 'only_tvshow_nfo')
        return (
            link_folders,
            target_folder,
            thread_count,
    ...
    def create_symlink(self):
        ...
        target_folder = self.config.get('symlink_export', 'target_folder')
        thread_count = self.config.get('symlink_export', 'thread_count')
        soft_link_extensions = tuple(self.config.get('symlink_export', 'link_suffixes'))
        enable_replace_path = self.config.get('symlink_export', 'enable_replace_path')
        original_path = self.config.get('symlink_export', 'original_path')
        replace_path = self.config.get('symlink_export', 'replace_path')
        ...
        # 运行元数据复制
        creator.run(on_create_symlink_complete)
