...
- **WebUI backend:** `python main.py --serve-web`
  ...
  - The WebUI `网盘同步` card is shown as a fixed checklist for `movies` and `tvshows`. Each row maps a local symlink workspace to a new cloud-side target directory. The card defaults to dry-run so first clicks do not write or move files. Real non-dry-run cloud import first validates each first-level media folder for complete scraped metadata, then attempts Stage B video movement only for symlinks in complete folders. Standalone cloud execution requires explicit confirmation because moving real videos makes the C-drive symlink workspace stale. While this task or its CloudDrive2 probe is active, the button becomes `取消执行` and cancellation must request cancellation for the active backend run.
...
- `scrape_metadata` — Metadata scraping workflow for the standardized symlink media library.
  ...
  - Movie scraping currently supports TMDB search/details, `zh-CN` metadata with `en-US` fallback for missing core fields, rule-based query expansion, known TMDB-ID direct matches, LLM-generated alias/original-title retry when TMDB returns no movie candidates, video-stem NFO writing, and poster/fanart/clearlogo downloading. Normal single-movie folders use the folder name as the search source; first-level folders without a year that contain multiple videos are treated as mixed/category folders, so each movie is searched from its own video filename instead of the parent folder name. Movie TMDB search text strips non-year bracket notes such as actor names or subtitle tags, while preserving year markers. Rule-based expansion tries known stylized aliases, romanization aliases, and Chinese title fragments before the raw title, such as `GirlS/Girls (2010)` -> `囡囡`, `JinChun (1993)` -> `禁春`, `18P2P色模SuperModels[...]` -> `色模`, `1983欲望之翼BD720P...` -> `欲望之翼`, `93女爱男欢（...）-93女爱男欢` -> `93女爱男欢`, `97风流梦（...）-97风流梦` -> `97风流梦`, and `三分之一情人（...）-三分之一情人` -> `三分之一情人`. Known direct matches cover TMDB search gaps such as `janinhan.miyongsaui (2014/2015)` -> TMDB `322587` and `96超级床上接班人` -> TMDB `926910`; local metadata overrides may preserve the user-verified title when TMDB's localized title omits a distinguishing prefix.
  - TV scraping currently supports TMDB search/details, episode details, `{tmdb-123}` direct ID tags found in the show folder or episode filenames, `zh-CN` metadata with `en-US` fallback for missing core fields, no-year retry when the first-air-year filter returns no candidates, LLM-generated alias/original-title retry when TMDB still returns no TV candidates, `tvshow.nfo`, episode NFO, show poster/fanart/clearlogo/logo/thumb/banner downloading, best-effort clearart downloading when the provider exposes clearart images, season poster/thumb downloading including Specials artwork, and episode thumbnail downloading.
...
- `build_cloud_scraped_library` — Build a cloud-side scraped library from the local symlink workspace.
  ...
  - Stage A mirrors only complete scraped metadata directories to the target and copies every non-symlink file while excluding symlink files/directories. Normal eligible video symlinks are added to the Stage B move plan without writing per-file `skip_symlink_copy` report rows, keeping reports focused on review-worthy skipped/failed items.
