...
- `build_cloud_scraped_library` — Build a cloud-side scraped library from the local symlink workspace.
  ...
  - A symlink video is eligible for Stage B only when the same directory contains a same-stem NFO file, for example `movie.mkv` requires `movie.nfo`. Missing same-stem NFO means metadata scraping did not complete for that video; do not create a real-video move plan even when `metadata-only` is disabled. Report the skipped symlink and mark the run `partial`.
