...
- **WebUI backend:** `python main.py --serve-web`
  ...
  - The WebUI has a one-click full-flow button that front-end orchestrates as `build_symlink_workspace` followed by one `scrape_metadata` run per checked symlink target. While the full flow is active, the button becomes `取消执行`; cancellation is safe/cooperative and stops launching later steps after the current backend step finishes. Do not add a separate CLI/full-flow backend action unless the product contract changes.
