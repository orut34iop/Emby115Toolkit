...
- `scrape_metadata` — Metadata scraping workflow for the standardized symlink media library.
  ...
  - TV scraping currently supports TMDB search/details, episode details, `{tmdb-123}` direct ID tags found in the show folder or episode filenames, `zh-CN` metadata with `en-US` fallback for missing core fields, no-year retry when the first-air-year filter returns no candidates, LLM-generated alias/original-title retry when TMDB still returns no TV candidates, `tvshow.nfo`, episode NFO, show poster/fanart/clearlogo downloading, season poster downloading, and episode thumbnail downloading.
