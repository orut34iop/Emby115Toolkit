...
- **WebUI backend:** `python main.py --serve-web`
  ...
  - The WebUI `网盘同步` card is shown as a fixed checklist for `movies` and `tvshows`. Each row maps a local symlink workspace to a new cloud-side target directory. The card defaults to dry-run so first clicks do not write or move files. Real non-dry-run cloud import always attempts Stage B video movement for symlinks that have same-stem NFO files, and standalone cloud execution requires explicit confirmation because moving real videos makes the C-drive symlink workspace stale. While this task or its CloudDrive2 probe is active, the button becomes `取消执行` and cancellation must request cancellation for the active backend run.
...
- `build_cloud_scraped_library` — Build a cloud-side scraped library from the local symlink workspace.
  ...
  - Stage A mirrors valid scraped metadata directories to the target and copies every non-symlink file while excluding symlink files/directories. For TV libraries, a first-level show directory must contain root `tvshow.nfo`; if it does not, skip the whole show directory, record `skip_tvshow_without_metadata`, and do not upload that folder or create video move plans for it.
  - A symlink video is eligible for Stage B only when the same directory contains a same-stem NFO file, for example `movie.mkv` requires `movie.nfo`. Missing same-stem NFO means metadata scraping did not complete for that video; do not create a real-video move plan in real runs. Report the skipped symlink and mark the run `partial`. Normal eligible video symlinks are added to the Stage B move plan without writing per-file `skip_symlink_copy` report rows, keeping reports focused on review-worthy skipped/failed items.
