"""Genre translation maps used by media server metadata updates."""

TV_GENRE_TRANSLATIONS = {
    'Action': '动作',
    'Adventure': '冒险',
    'Animation': '动画',
    'Biography': '传记',
    'Comedy': '喜剧',
    'Crime': '犯罪',
    'Documentary': '纪录片',
    '纪录': '纪录片',
    'Drama': '剧情',
    'Family': '家庭',
    'Fantasy': '奇幻',
    'Food': '美食',
    'Game Show': '游戏节目',
    'History': '历史',
    'Holiday': '节日',
    'Horror': '恐怖',
    'Mini-Series': '迷你剧',
    'Music': '音乐',
    'Musical': '音乐剧',
    'Mystery': '悬疑',
    'Reality': '纪实',
    'Reality TV': '真人秀',
    '真人秀电视': '真人秀',
    'Romance': '爱情',
    '浪漫': '爱情',
    'Sci-Fi & Fantasy': '科幻与奇幻',
    'Science Fiction': '科幻',
    'Short': '短片',
    'Soap': '肥皂剧',
    'Sport': '运动',
    'Suspense': '悬疑',
    'Talk Show': '脱口秀',
    'Thriller': '惊悚',
    'Travel': '旅行',
    'War': '战争',
    'Western': '西部',
    'Animal': '动物',
    'War & Politics': '战争与政治',
}

MOVIE_GENRE_TRANSLATIONS = {
    'Action': '动作',
    'Adult': '成人',
    'Adventure': '冒险',
    'Animation': '动画',
    'Anime': '动漫',
    'Biography': '传记',
    'Children': '儿童',
    'Comedy': '喜剧',
    'Crime': '犯罪',
    'Documentary': '纪录片',
    '纪录': '纪录片',
    'Drama': '剧情',
    'Eastern': '东方',
    'Erotic': '情色',
    'Family': '家庭',
    'Fantasy': '奇幻',
    '魔幻': '奇幻',
    'Film Noir': '黑色影片',
    'History': '历史',
    'Holiday': '节日',
    'Horror': '恐怖',
    'Indie': '独立影片',
    'Martial Arts': '功夫',
    'Music': '音乐',
    'Musical': '音乐剧',
    'Mystery': '悬疑',
    '悬念': '悬疑',
    'News': '新闻',
    'Reality TV': '真人秀',
    'Reality': '真人秀',
    '真人秀电视': '真人秀',
    'Romance': '爱情',
    '浪漫': '爱情',
    'Science Fiction': '科幻',
    'Short': '短片',
    'Sport': '运动',
    'Suspense': '悬疑',
    'TV Movie': '电视电影',
    'Thriller': '惊悚',
    'War': '战争',
    'Western': '西部',
    'superhero': '超级英雄',
    'Animal': '动物',
    'War & Politics': '战争与政治',
    '拷問': '折磨',
    'ゲロ': '呕吐',
    '触手': '触手',
    'ツンデレ': '蛮横娇羞',
    '童貞': '处男',
    'ショタ': '正太控',
    '寝取り・寝取られ': '绿帽/戴绿帽',
    'くすぐり': '瘙痒',
    'スポーツ': '运动',
    'レズキス': '女同接吻',
    'セクシー': '性感',
    'エステ': '美容院',
    '処女': '处女',
    '泥酔': '烂醉如泥的',
    '残虐表現': '残忍画面',
    '妄想': '妄想',
    'イタズラ': '恶作剧',
    '学園もの': '学校作品',
    '鬼畜': '粗暴',
    '不倫': '通姦',
    '姉・妹': '姐妹',
    'ふたなり': '双性人',
    'ダンス': '跳舞',
    '調教・奴隷': '性奴',
    '逆ナン': '倒追',
    '痴漢': '性骚扰',
    'その他': '其他',
    '脚フェチ': '恋腿癖',
    '盗撮・のぞき': '偷窥',
    '淫乱・ハード系': '花痴',
    'ゲイ・ホモ': '男同性恋',
    'カップル': '情侣',
    '巨乳フェチ': '巨乳恋物',
    '近親相姦': '乱伦',
    'その他フェチ': '其他恋物癖',
    'アイドル・芸能人': '偶像艺人',
    '野外・露出': '野外・露出',
    'ナンパ': '猎艳',
    'レズ': '女同性恋',
    '企画': '企划',
    '10枚組': '10枚组',
    'SF': '科幻',
    '女優ベスト・総集編': '最佳女演员/合集',
    '温泉': '温泉',
    'M男': '受虐狂',
    '原作コラボ': '原作合作',
    '16時間以上作品': '16时间以上作品',
    'デカチン・巨根': '巨根',
    'ファン感謝・訪問': '粉丝答谢/探访',
    '動画': '动画',
    '巨尻': '巨臀',
    'ハーレム': '后宫',
    '日焼け': '日光浴',
    '早漏': '早泄',
    'キス・接吻': '接吻',
    '汗だく': '汗流浃背',
    'スマホ専用縦動画': '竖屏视频',
    'Vシネマ': '影碟首发电影',
    "Don Cipote's choice": '唐·西波特的精选',
    'アニメ': '动漫',
    'アクション': '动作',
    'イメージビデオ（男性）': '男性形象视频',
    '孕ませ': '怀孕',
    'ボーイズラブ': '耽美',
    'ビッチ': '婊子',
    '特典あり（AVベースボール）': '附赠特典（AV）',
    'コミック雑誌': '漫画杂志:',
    '時間停止': '时间停止',
    '極道・任侠': '黑帮成员',
    '幼なじみ': '童年朋友',
    'お姫様': '公主',
    'アジア女優': '亚洲女演员',
    'コンパニオン': '伴侣',
    'インストラクター': '讲师',
    '義母': '继母/义母',
    'アクション・格闘': '格斗家',
    '女捜査官': '女检察官',
    'そっくりさん': '明星脸',
    '女将・女主人': '女主人、女老板',
    'モデル': '模特儿',
    '秘書': '秘书',
    '美少女': '美少女',
    '花嫁・若妻': '新娘、年轻妻子',
    'お姉さん': '轻熟女/姐姐',
    '格闘家': '格斗家',
    'バスガイド': '车掌小姐',
    '未亡人': '寡妇',
    'お嬢様・令嬢': '年轻女士',
    '白人女優': '白人',
    '人妻': '人妻',
    '女医': '女医生',
    '職業色々': '各种职业',
    'キャバ嬢・風俗嬢': '妓女',
    'レースクィーン': '赛车女郎',
    '女子大生': '女大学生',
    'キャンギャル': '展场女孩',
    '女教師': '女教师',
    'お母さん': '母亲',
    '家庭教師': '家教',
    '看護婦・ナース': '护士',
    '痴女': '痴女',
    '黒人男優': '黑人演员',
    'ギャル': '辣妹',
    '女子アナ': '女主播',
    '女子校生': '女子校生',
    'ウェイトレス': '服务生',
    '魔法少女': '魔法少女',
    '学生（その他）': '学生（其他）',
    'アニメキャラクター': '动画人物',
    'ゲーム実写版': '游戏的真人版',
    '変身ヒロイン': '超级女英雄',
    '女戦士': '女战士',
    'ニーソックス': '及膝袜',
    'ドール': '娃娃',
    'くノ一': '女忍者',
    '女装・男の娘': '女装人妖',
    'パンチラ': '走光/内裤走光',
    '着エロ': '猥褻穿著',
    'バニーガール': '兔女郎',
    'ネコミミ・獣系': '猫耳女',
    '巫女': '女祭司',
    'ルーズソックス': '泡泡袜',
    '制服': '制服',
    'レオタード': '紧身衣',
    '裸エプロン': '裸体围裙',
    'ミニスカポリス': '迷你裙警察',
    'スチュワーデス': '空中小姐',
    'パンスト': '连裤袜',
    'ボディコン': '紧身裙',
    'OL': '职场女性',
    '和服・浴衣': '和服/浴衣',
    '体操着・ブルマ': '体育服',
    'ランジェリー': '内衣',
    'セーラー服': '水手服',
    '競泳・スクール水着': '学校泳装',
    'チャイナドレス': '旗袍',
    'メイド': '女佣',
    'ミニスカ': '迷你裙',
    '学生服': '校服',
    '水着': '泳装',
    'めがね': '眼镜',
    'コスプレ': '角色扮演',
    'ゴシックロリータ': '哥德萝莉',
    '超乳': '超乳',
    '筋肉': '肌肉',
    '美乳': '美乳',
    'ミニ系・小柄': '娇小的',
    '尻フェチ': '屁股',
    '長身': '高挑',
    'ニューハーフ': '变性者',
    'パイパン': '无毛',
    'ぽっちゃり': '丰满',
    'スレンダー': '苗条',
    '妊婦': '孕妇',
    '熟女': '熟女',
    'ロリ系': '萝莉塔',
    '貧乳・微乳': '小胸',
    '巨乳': '巨乳',
    '顔面騎乗': '颜面骑乘',
    '食糞': '食粪',
    '足コキ': '足交',
    '母乳': '母乳',
    '指マン': '手指插入',
    'マッサージ': '按摩',
    '騎乗位': '女上位',
    'クンニ': '舔阴',
    'フィスト': '拳交',
    'イラマチオ': '深喉',
    'シックスナイン': '69',
    '淫語': '淫语',
    '潮吹き': '潮吹',
    'パイズリ': '乳交',
    '脱糞': '排便',
    '飲尿': '饮尿',
    'フェラ': '口交',
    '乱交': '滥交',
    '放尿': '放尿',
    '手コキ': '打手枪',
    'ごっくん': '吞精',
    'アナル': '肛交',
    'ぶっかけ': '颜射',
    'オナニー': '自慰',
    '顔射': '颜射',
    '中出し': '中出',
    'アナルセックス': '肛内中出',
    '即ハメ': '立即性爱',
    '電マ': '电动按摩棒',
    'ポルチオ': '子宫颈',
    '催眠・洗脳': '催眠',
    'ローション': '乳液',
    '羞恥': '羞耻',
    '辱め': '凌辱',
    '拘束': '捆绑',
    '輪姦': '轮姦',
    '異物挿入': '插入异物',
    'クスコ': '鸭嘴',
    '浣腸': '灌肠',
    '監禁': '监禁',
    '縛り・緊縛': '紧缚',
    '強姦': '强姦',
    'ドラッグ': '药物',
    'カーセックス': '汽车性爱',
    'SM': 'SM',
    'スカトロ': '粪便',
    'おもちゃ': '玩具',
    'ローター': '跳蛋',
    'ボンテージ': '紧缚',
    'バイブ': '按摩棒',
    '3P・4P': '多P',
    'エロス': '情色',
    'ディルド': '假阳具',
    '逆レイプ': '逆强奸',
    'コラボ作品': '合作作品',
    'ホラー': '恐怖',
    '女性向け': '给女性观眾',
    'How To': '教学',
    'DMM配信限定': 'DMM专属',
    'R-15': 'R-15',
    'R-18': 'R-18',
    '3D': '3D',
    '特撮': '特效',
    '複数話': '多集',
    '期間限定セール': '限时降价',
    '復刻': '复刻版',
    'ドラマ': '戏剧',
    '恋愛': '恋爱',
    'ハイビジョン': '高画质',
    '主観': '主观视角',
    'イメージビデオ': '介绍影片',
    '4時間以上作品': '4小时以上作品',
    'ギリモザ': '薄马赛克',
    'クラシック': '经典',
    'デビュー作品': '首次亮相',
    'デジモ': '数位马赛克',
    '投稿': '投稿',
    'ドキュメンタリー': '纪录片',
    '洋ピン・海外輸入': '国外进口',
    'ハメ撮り': '第一人称摄影',
    '素人': '素人',
    '局部アップ': '局部特写',
    'インディーズ': '独立製作',
    '独占配信': 'DMM独家',
    '単体作品': '单体作品',
    'ベスト・総集編': '合集',
    '高解像度': '高清',
    '字幕': '字幕',
    'パラダイスTV': '天堂TV',
    'DVDトースター': 'DVD多士炉',
    'AV OPEN 2014 スーパーヘビー': 'AV OPEN 2014 超重量级',
    'AV OPEN 2014 ヘビー級': 'AV OPEN 2014 重量级',
    'AV OPEN 2014 ヘビー级': 'AV OPEN 2014 重量级',
    'AV OPEN 2014 ミドル級': 'AV OPEN 2014 中量级',
    'AV OPEN 2015 マニア/フェチ部門': 'AV OPEN 2015 恋物癖部门',
    'AV OPEN 2015 熟女部門': 'AV OPEN 2015 熟女部门',
    'AV OPEN 2015 企画部門': 'AV OPEN 2015 企划部门',
    'AV OPEN 2015 乙女部門': 'AV OPEN 2015 少女部门',
    'AV OPEN 2015 素人部門': 'AV OPEN 2015 素人部门',
    'AV OPEN 2015 SM/ハード部門': 'AV OPEN 2015 SM/硬核部门',
    'AV OPEN 2015 女優部門': 'AV OPEN 2015 女优部门',
    'AVOPEN2016人妻・熟女部門': 'AVOPEN2016人妻/熟女部门',
    'AVOPEN2016企画部門': 'AVOPEN2016企划部门',
    'AVOPEN2016ハード部門': 'AVOPEN2016硬核部门',
    'AVOPEN2016マニア・フェチ部門': 'AVOPEN2016恋物癖部门',
    'AVOPEN2016マニア·フェチ部门': 'AVOPEN2016恋物癖部门',
    'AVOPEN2016乙女部門': 'AVOPEN2016少女部门',
    'AVOPEN2016女優部門': 'AVOPEN2016女优部门',
    'AVOPEN2016ドラマ・ドキュメンタリー部門': 'AVOPEN2016剧情/纪录片部门',
    'AVOPEN2016ドラマ·ドキュメンタリー部门': 'AVOPEN2016剧情/纪录片部门',
    'AVOPEN2016素人部門': 'AVOPEN2016素人部门',
    'AVOPEN2016バラエティ部門': 'AVOPEN2016综艺部门',
    'AVOPEN2016バラエティ部门': 'AVOPEN2016综艺部门',
    'VR専用': 'VR专用',
    'ギャグ・コメディ': '恶搞喜剧',
    'ファンタジー': '幻想',
    '性転換・女体化': '性别转型·女性化',
    'スマホ推奨縦動画': '竖屏视频',
    'セット商品': '系列合集',
    'ミニ系': '迷你',
    '体験告白': '体验懺悔',
    'ダーク系': '黑暗系统',
    'オナサポ': '自慰支持',
    'アスリート': '运动员',
    '覆面・マスク': '蒙面',
    'ハイクオリティVR': '高品质VR',
    'ヘルス・ソープ': '风俗/泡泡浴',
    'ホテル': '酒店',
    'アクメ・オーガズム': '高潮',
    '花嫁': '新娘',
    'デート': '约会',
    '軟体': '柔软的身体',
    '娘・養女': '女儿与继女',
    'スパンキング': '打屁股',
    'スワッピング・夫婦交換': '交换伴侣与换夫',
    '部下・同僚': '下属与同事',
    '旅行': '旅行',
    '胸チラ': '露乳',
    'バック': '后入式',
    '男の潮吹き': '男性喷水',
    '女上司': '女老板',
    '受付嬢': '接待员',
    'ノーブラ': '不穿胸罩',
    '白目・失神': '翻白眼与晕厥',
    'M女': '受虐狂女性',
    '女王様': '女虐待狂',
    'ノーパン': '不穿内裤',
    'セレブ': '名人',
    '病院・クリニック': '医院与诊所',
    '面接': '采访',
    'お風呂': '浴室',
    '叔母さん': '阿姨',
    '罵倒': '辱骂/责骂',
    'お爺ちゃん': '爷爷',
    'ヨガ': '瑜伽',
    '飲み会・合コン': '饮酒派对与集体约会',
    '部活・マネージャー': '俱乐部活动与经理',
    'お婆ちゃん': '奶奶',
    'ビジネススーツ': '职业套装',
    'チアガール': '啦啦队长',
    'ママ友': '妈妈朋友',
    'エマニエル': '艾曼纽',
    '妄想族': '妄想家庭',
    '蝋燭': '蜡烛',
    '鼻フック': '鼻钩',
    '放置': '放置',
    'サンプル動画': '样品动画',
    'サイコ・スリラー': '心理惊悚',
    'ラブコメ': '浪漫喜剧',
    'オタク': '宅男宅女',
    '4K': '4K',
    '福袋': '福袋',
    '玩具責め': '玩具虐待',
    '女優': '女优',
    'お掃除フェラ': '清洁口交',
    '筆おろし': '初体验',
    '美脚': '美腿',
    '美尻': '美臀',
    'フェチ': '恋物癖',
    '羞恥プレイ': '羞耻游戏',
    '逆ナンパ': '反向搭讪',
    '着衣': '着装',
    '誘惑': '诱惑',
    'チラリズム': '若隐若现/走光',
    '写真集': '写真集',
    '書籍版': '书籍版',
    'SM拘束': 'SM束缚',
    'ドキュメント': '纪录片',
    'グッズ': '周边',
    'Tシャツ': 'T恤',
    '失禁': '失禁',
    'オナホール': '飞机杯',
    'カップホール': '杯洞',
    '非貫通': '非实质插入',
    'ローション付き': '含润滑液',
    '配信専用': '仅限流媒体播放',
    '官能小説': '情色小说',
    'CD': 'CD',
    '生挿入': '无套插入',
    '風俗': '卖淫',
    '外国人': '外国人',
    '淫語責め': '污言秽语',
    'アナルファック': '肛交',
    '脚コキ': '足交',
    'オイルプレイ': '油浴游戏',
    'キャバ嬢': '陪酒女',
    '調教': '调教',
    'ローション・オイル': '润肤露/油',
    '媚薬': '催情剂',
    '顔面騎乗位': '女上位颜射',
    '微乳': '小胸',
    'デカチン': '大阴茎',
    '3P、4P': '多P',
    '野外': '户外',
    '露出': '暴露癖',
    'レイプ': '强奸',
    'ベスト、総集編': '最佳合集',
    'マジックミラー号': '魔镜面包车',
    '看護師': '护士',
    'アイドル': '偶像艺人',
    '妹': '妹妹',
    'スクール水着': '学校泳装',
    'デリヘル': '送货服务',
    '凌辱': '凌辱',
    '童貞モノ': '处女',
    'ブルマ': '运动短裤/灯笼裤',
    'ホテヘル': '酒店',
    '縛り': '捆绑',
    'ロリータ': '洛丽塔',
    'ミニスカート': '迷你裙',
    '尻': '屁股',
    '奴隷': '奴隶',
    '芸能人': '艺能人',
    '下着': '内衣',
    '母親': '母亲',
    '湯': '浴室',
    '教師': '教师',
    'ストッキング': '长筒袜/丝袜',
    '電車、バス': '电车、公交车',
    '健康診断': '健康检查',
    '野球拳': '野球拳',
    '浴衣': '和服/浴衣',
    '爆乳': '巨乳',
    'ソープ': '泡泡浴',
    'スワッピング': '换妻俱乐部',
    'メガネ': '眼镜',
    '素股': '素股',
    'ドリンク': '饮料',
    'フルハイビジョン(FHD)': '满的高清',
    '三十路': '30多岁',
    '羞恥・辱め': '羞耻/羞辱',
    'パンチラモノ': '裙底偷拍',
    '放尿・失禁': '小便/尿失禁',
    '五十路': '50多岁',
    'ショートヘアー': '短发',
    '着物・浴衣': '和服/浴衣',
    '清楚': '清纯',
    '四十路': '40多岁',
    'VR': 'VR',
    'オモチャ': '玩具',
    'エステ・マッサージ': '美容院/按摩',
    'Gカップ': 'G罩杯',
    '目隠し': '蒙眼',
    'ドラッグ・媚薬': '药物/催情剂',
    '六十路': '60多岁',
    '淫語モノ': '污言秽语',
    '口内発射': '口内射精',
    '童顔': '娃娃脸',
    'スチュワーデス・CA': '空姐/CA',
    'Dカップ': 'D罩杯',
    '美熟女': '美丽成熟女性',
    'MGSだけのおまけ映像付き': 'MGS独家花絮',
    '手マン': '手指爱抚',
    'パンストモノ': '丝袜',
    'フェラモノ': '口交',
    '性教育': '性教育',
    'HowTo': '教学',
    'MGS限定特典映像': 'MGS独家花絮',
    '寝取り･寝取られ': '绿帽/戴绿帽',
    'Eカップ': 'E罩杯',
    'Fカップ': 'F罩杯',
    'ロングヘアー': '长发',
    '黒髪': '黑发',
    'ショートヘア': '短发',
    '金髪': '金发',
    '茶髪': '棕发',
    '女子高生': '高中女生',
    'ハーフ': '混血',
    '金髪・ブロンド': '金发',
    '初撮り': '初次拍摄',
    '介護': '护理',
    'Hカップ': 'H罩杯',
    '期間限定販売': '限时特惠',
    '多人数': '群交',
    '風俗嬢': '妓女',
    'PICKUP素人': '搭讪业余',
    'オイル・ローション': '油/乳液',
    '8KVR': '8K VR',
    'プレステージ20周年特別企画': '尊贵20周年特别版',
    '口内射精': '口内射精',
    '美女': '美女',
    'MGSだけの特典映像付': 'MGS独家花絮',
    '色白': '白皙肌肤',
    '総集編・ベスト': '合集/精选',
    '近●相●': '乱伦',
    'ハイビジョン(HD)': '高清 (HD)',
    '配信専用素人': '仅限流媒体播放的业余作品',
    '台湾モデル': '台湾模特',
    'ショートカット': '短发',
    '小柄': '娇小',
    'くびれ': '沙漏型身材',
    'ミディアムヘアー': '中长发',
    'AI生成作品': 'AI生成作品',
    '個人撮影': '个人拍摄',
    'Torture': '折磨',
    'Vomit': '呕吐',
    'Tentacle': '触手',
    'TSUNDERE': '蛮横娇羞',
    'Cherry Boy': '处男',
    'Shotacon': '正太控',
    'Cheating Wife': '绿帽/戴绿帽',
    'Tickling': '瘙痒',
    'Sports': '运动',
    'Lesbian Kissing': '女同接吻',
    'Sexy': '性感的',
    'Massage Parlor': '美容院',
    'Virgin': '处女',
    'Drunk Girl': '烂醉如泥的',
    'Cruelty': '残忍画面',
    'Daydream': '妄想',
    'Pranks': '恶作剧',
    'School': '学校作品',
    'Rough Sex': '粗暴',
    'Adultery': '通姦',
    'Older & Younger Sister': '姐妹',
    'Hermaphrodite': '双性人',
    'Dance': '跳舞',
    'Sex Slave': '性奴',
    'Reverse Pick Up': '倒追',
    'Molester': '性骚扰',
    'Others': '其他',
    'Foot Fetish': '恋腿癖',
    'Voyeur': '偷窥',
    'Nymphomaniac': '花痴',
    'Gay': '男同性恋',
    'Couple': '情侣',
    'Big Tits Lover': '恋乳癖',
    'Incest': '乱伦',
    'Other Fetishes': '其他恋物癖',
    'Idol & Celebrity': '偶像艺人',
    'Outdoor': '野外・露出',
    'Picking Up Girls': '猎艳',
    'Lesbian': '女同性恋',
    'Variety': '企画',
    'Ten Clips': '10枚组',
    'Sci-Fi': '科幻',
    'Actress Best Compilation': '最佳女演员/合集',
    'Hot Spring': '温泉',
    'Masochist Man': '受虐狂',
    'Original Collaboration': '与原创作品合作',
    'More Than 16 Hours Of Footage': '16时间以上作品',
    'Huge Dick - Large Dick': '巨根',
    'Fan Appreciation/ Home Visit': '粉丝答谢/探访',
    'Big Asses': '大屁股',
    'Suntan': '日光浴',
    'Premature Ejaculation': '早泄',
    'Kiss Kiss': '接吻',
    'Sweating': '汗流浃背',
    'Special Smartphone Vertical Video': '竖屏视频',
    'Pretty Boy': '男性形象视频',
    'BL': '耽美',
    'Time Freezing': '时间停止',
    'YAKUZA': '黑帮成员',
    'Childhood Friend': '童年朋友',
    'Princess': '公主',
    'Asian Actress': '亚洲女演员',
    'Hostess': '伴侣',
    'Instructor': '讲师',
    'Stepmom': '继母/义母',
    'Action & Fighting': '格斗家',
    'Female Detective': '女检察官',
    'Lookalike': '明星脸',
    'Housewife': '女主人、女老板',
    'Model': '模特儿',
    'Secretary': '秘书',
    'Beautiful Girl': '美少女',
    'Young Wife': '新娘、年轻妻子',
    'Older Sister': '姐姐',
    'Bus Tour Guide': '车掌小姐',
    'Widow': '寡妇',
    'Princess & Mademoiselle': '千金小姐',
    'Caucasian Actress': '白人',
    'Married Woman': '已婚妇女',
    'Female Doctor': '女医生',
    'Various Worker': '各种职业',
    'Club Hostess & Sex Worker': '妓女',
    'Race Queen': '赛车女郎',
    'College Girl': '女大学生',
    'Campaign Girl': '展场女孩',
    'Female Teacher': '女教师',
    'MILF': '母亲',
    'Private Tutor': '家教',
    'Nurse': '护士',
    'Slut': '荡妇',
    'Black Man': '黑人演员',
    'Gal': '女生',
    'Female Anchor': '女主播',
    'Schoolgirl': '高中女生',
    'Waitress': '服务生',
    'Magical Girl': '魔法少女',
    'Student (Other)': '学生（其他）',
    'Anime Character': '动画人物',
    'Live Game Filming Edition': '游戏的真人版',
    'Super Heroine': '超级女英雄',
    'Female Soldier': '女战士',
    'Knee-High Socks': '及膝袜',
    'Doll': '娃娃',
    'Female Ninja': '女忍者',
    'Cross Dresser': '女装人妖',
    'Panty Shot': '走光/内裤走光',
    'Non-nude Erotica': '猥褻穿著',
    'Bunny Girl': '兔女郎',
    'Animal Ears': '猫耳女',
    'Priestess': '女祭司',
    'Loose Socks': '泡泡袜',
    'Uniform': '制服',
    'Leotards': '紧身衣',
    'Naked Apron': '裸体围裙',
    'Miniskirt Police': '迷你裙警察',
    'Stewardess': '空中小姐',
    'Pantyhose': '连裤袜',
    'Tight Dress': '紧身裙',
    'Office Lady': 'OL',
    'KIMONO': '和服/浴衣',
    'Gym Clothes': '体育服',
    'Lingerie': '内衣',
    'Sailor Uniform': '水手服',
    'School Swimsuits': '学校泳装',
    'Chinese Dress': '旗袍',
    'Maid': '女佣',
    'Miniskirt': '迷你裙',
    'School Uniform': '校服',
    'Swimsuits': '泳装',
    'Glasses': '眼镜',
    'Cosplay': '角色扮演',
    'GothLoli': '哥德萝莉',
    'Huge Tits': '超乳',
    'Muscular': '肌肉',
    'Beautiful Tits': '乳房',
    'Short & Petite': '娇小的',
    'Ass Lover': '屁股',
    'Tall Girl': '高挑/高个子',
    'Transsexual': '变性者',
    'Shaved Pussy': '无毛',
    'Chubby': '丰满/微胖',
    'Slender': '苗条',
    'Pregnant': '孕妇',
    'Mature Woman': '成熟的女人',
    'Lolicon': '萝莉塔',
    'Small Tits': '贫乳/微乳',
    'Big Tits': '巨乳',
    'Face Sitting': '颜面骑乘',
    'Scat': '粪便',
    'Footjob': '足交',
    'Breast Milk': '母乳',
    'Fingering': '手指插入',
    'Massage': '按摩',
    'Cowgirl': '女上位',
    'Cunnilingus': '舔阴',
    'Fisting': '拳交',
    'Deep Throat': '深喉',
    '69': '69',
    'Dirty Talk': '淫语',
    'Squirting': '潮吹',
    'Titty Fuck': '乳交',
    'Pooping': '排便',
    'Golden Shower': '饮尿',
    'Blowjob': '口交',
    'Orgy': '滥交',
    'Urination': '放尿',
    'Handjob': '打手枪',
    'Cum Swallowing': '吞精',
    'Anal': '肛交',
    'BUKKAKE': '颜射',
    'Masturbation': '自慰',
    'Facial': '颜射',
    'Creampie': '中出',
    'AF': '肛内中出',
    'Quickie': '立即性爱',
    'Big Vibrator': '电动按摩棒',
    'G-Spot': 'G点',
    'Hypnotism': '催眠',
    'Lotion': '乳液',
    'Shame': '羞耻',
    'Torture & Rape': '凌辱',
    'Tied Up': '拘束',
    'Gang Bang': '轮姦',
    'Object Insertion': '插入异物',
    'Gyno Exam': '鸭嘴',
    'Enema': '灌肠',
    'Confinement': '监禁',
    'Bondage': '紧缚',
    'Rape': '强姦',
    'Drugs': '药物',
    'Car Sex': '汽车性爱',
    'BDSM': 'SM',
    'Sex Toys': '玩具',
    'Egg Vibrator': '跳蛋',
    'Vibrator': '按摩棒',
    'Threesome / Foursome': '多P',
    'Eros': '性爱',
    'Dildo': '假阳具',
    'Reverse Rape': '逆强奸',
    'Collaboration work': '合作作品',
    'Boys Love': '给女性观眾',
    'DMM Exclusive': 'DMM独家',
    'Restricted to Under ': 'R-18',
    'Special Effects': '特效',
    'Series': '故事集',
    'Sale (limited time)': '限时降价',
    'Reprint': '复刻版',
    'Love': '恋爱',
    'Hi-Def': '高画质',
    'POV': '第一人称摄影',
    'Idol Video': '介绍影片',
    'Over 4 Hours': '4小时以上作品',
    'Minimal Mosaic': '薄马赛克',
    'Classics': '经典',
    'Debut': '首次亮相',
    'Digital Mosaic': '数位马赛克',
    'Homemade': '投稿',
    'Foreign Imports': '国外进口',
    'Amateur': '业餘',
    'Genital Close-Up': '局部特写',
    'Independent': '独立製作',
    'Featured Actress': '单体作品',
    'Compilation': '合集',
    'High Definition': '高清',
    'Subtitle': '字幕',
    'Paradise TV': '天堂TV',
    'DVD Toaster': 'DVD多士炉',
    'AV OPEN 2015 Kink/Fetish Division': 'AV OPEN 2015 マニア/フェチ部门',
    'AV OPEN 2015 Cougar Division': 'AV OPEN 2015 熟女部门',
    'AV OPEN 2015 Variety Show Division': 'AV OPEN 2015 企画部门',
    'AV OPEN 2015 Virgin Division': 'AV OPEN 2015 乙女部门',
    'AV OPEN 2015 Amateur Division': 'AV OPEN 2015 素人部门',
    'AV OPEN 2015 S&M/Hardcore Division': 'AV OPEN 2015 SM/ハード部门',
    'AV OPEN 2015 Porn Star Division': 'AV OPEN 2015 女优部门',
    'AV OPEN 2016 Kink/Fetish Division': '2016 AV大赏人妻・熟女作品',
    'AV OPEN 2016 Variety Show Division': 'AVOPEN2016企画部门',
    'AV OPEN 2016 S&M/Hardcore Division': 'AVOPEN2016ハード部门',
    'AV OPEN 2016 Virgin Division': 'AVOPEN2016乙女部门',
    'AV OPEN 2016 Porn Star Division': 'AVOPEN2016女优部门',
    'AV OPEN 2016 Amateur Division': 'AVOPEN2016素人部门',
    'Gag·Comedy': '喜剧',
    'Gender transformation·feminization': '性别转型·女性化',
    'Recommended vertical video for smartphone': '竖屏视频',
    'Set items': '系列合集',
    'Mini series': '迷你系列',
    'Experience confession': '体验懺悔',
    'Dark system': '黑暗系统',
    '折磨': '折磨',
    '嘔吐': '呕吐',
    '觸手': '触手',
    '蠻橫嬌羞': '蛮横娇羞',
    '處男': '处男',
    '正太控': '正太控',
    '出軌': '绿帽/戴绿帽',
    '瘙癢': '瘙痒',
    '運動': '运动',
    '女同接吻': '女同接吻',
    '性感的': '性感',
    '美容院': '美容院/按摩',
    '處女': '处女',
    '爛醉如泥的': '烂醉如泥的',
    '殘忍畫面': '残忍画面',
    '惡作劇': '恶作剧',
    '學校作品': '学校作品',
    '粗暴': '粗暴',
    '通姦': '通奸',
    '姐妹': '姐妹',
    '雙性人': '双性人',
    '跳舞': '跳舞',
    '性奴': '性奴',
    '倒追': '倒追',
    '性騷擾': '性骚扰',
    '其他': '其他',
    '戀腿癖': '恋腿癖',
    '偷窥': '偷窥',
    '花癡': '花痴',
    '男同性恋': '男同性恋',
    '情侶': '情侣',
    '戀乳癖': '恋乳癖',
    '乳房恋物': '恋乳癖',
    '亂倫': '乱伦',
    '其他戀物癖': '其他恋物癖',
    '偶像藝人': '偶像艺人',
    '獵豔': '猎艳',
    '女同性戀': '女同性恋',
    '企畫': '企画',
    '科幻': '科幻',
    '黑幫成員': '黑帮成员',
    '童年朋友': '青梅竹马',
    '公主': '公主',
    '亞洲女演員': '亚洲女演员',
    '伴侶': '伴侣',
    '講師': '讲师',
    '婆婆': '继母/义母',
    '格鬥家': '格斗家',
    '女檢察官': '女检察官',
    '明星臉': '明星脸',
    '女主人、女老板': '女老板',
    '模特兒': '模特儿',
    '新娘、年輕妻子': '新娘、年轻妻子',
    '姐姐': '轻熟女/姐姐',
    '車掌小姐': '车掌小姐',
    '寡婦': '寡妇',
    '千金小姐': '千金小姐',
    '白人': '白人',
    '已婚婦女': '已婚妇女',
    '女醫生': '女医生',
    '各種職業': '各种职业',
    '妓女': '风俗娘/性工作者',
    '賽車女郎': '赛车女郎',
    '女大學生': '女大学生',
    '展場女孩': '展场女孩',
    '家教': '家庭教师',
    '护士': '护士',
    '蕩婦': '荡妇',
    '黑人演員': '黑人演员',
    '女生': '女生',
    '女主播': '女主播',
    '高中女生': '高中女生',
    '服務生': '服务生',
    '學生（其他）': '学生（其他）',
    '動畫人物': '动画人物',
    '遊戲的真人版': '游戏的真人版',
    '超級女英雄': '超级女英雄',
    '女戰士': '女战士',
    '及膝襪': '及膝袜',
    '娃娃': '娃娃',
    '女忍者': '女忍者',
    '女裝人妖': '女装人妖',
    '內衣': '内衣',
    '猥褻穿著': '猥亵穿着',
    '兔女郎': '兔女郎',
    '貓耳女': '猫耳女',
    '女祭司': '女祭司',
    '泡泡襪': '泡泡袜',
    '緊身衣': '紧身衣',
    '裸體圍裙': '裸体围裙',
    '迷你裙警察': '迷你裙警察',
    '空中小姐': '空姐',
    '連褲襪': '连裤袜',
    '身體意識': '身体意识',
    '和服・喪服': '和服・丧服',
    '體育服': '体育服',
    '水手服': '水手服',
    '學校泳裝': '学校泳装',
    '旗袍': '旗袍',
    '女傭': '女佣',
    '迷你裙': '迷你裙',
    '校服': '校服',
    '泳裝': '泳装',
    '眼鏡': '眼镜',
    '角色扮演': '角色扮演',
    '哥德蘿莉': '哥德萝莉',
    '肌肉': '肌肉',
    '乳房': '乳房',
    '嬌小的': '娇小的',
    '屁股': '臀部',
    '高': '高挑',
    '變性者': '变性者',
    '無毛': '无毛',
    '胖女人': '丰满/微胖',
    '苗條': '苗条',
    '孕婦': '孕妇',
    '成熟的女人': '熟女',
    '蘿莉塔': '萝莉塔',
    '顏面騎乘': '颜面骑乘',
    '足交': '足交',
    '手指插入': '指法/手指插入',
    '按摩': '按摩',
    '女上位': '骑乘位',
    '舔陰': '舔阴',
    '拳交': '拳交',
    '深喉': '深喉',
    '潮吹': '潮吹',
    '乳交': '乳交',
    '排便': '排便',
    '口交': '口交',
    '濫交': '滥交',
    '打手槍': '打手枪',
    '吞精': '吞精',
    '肛交': '肛交',
    '顏射': '颜射',
    '自慰': '自慰',
    '中出': '中出',
    '肛内中出': '肛交',
    '立即口交': '立即性爱',
    '女優按摩棒': '电动按摩棒',
    '子宮頸': '子宫颈',
    '催眠': '催眠',
    '乳液': '润滑油',
    '插入異物': '插入异物',
    '鴨嘴': '鸭嘴',
    '灌腸': '灌肠',
    '紧缚': '捆绑',
    '藥物': '药物',
    '汽車性愛': '汽车性爱',
    '糞便': '粪便',
    '玩具': '玩具',
    '跳蛋': '跳蛋',
    '緊縛': '捆绑',
    '按摩棒': '按摩棒',
    '多P': '多P',
    '性愛': '性爱',
    '假陽具': '假阳具',
    '逆強姦': '逆强奸',
    '合作作品': '合作作品',
    '恐怖': '恐怖',
    '給女性觀眾': '给女性观眾',
    '教學': '教学',
    'DMM專屬': 'DMM专属',
    '特效': '特效',
    '故事集': '故事集',
    '限時降價': '限时降价',
    '複刻版': '复刻版',
    '戲劇': '戏剧',
    '戀愛': '恋爱',
    '高畫質': '高画质',
    '主觀視角': '第一视角',
    '介紹影片': '介绍影片',
    '4小時以上作品': '4小时以上作品',
    '薄馬賽克': '薄马赛克',
    '經典': '经典',
    '首次亮相': '出道',
    '數位馬賽克': '数位马赛克',
    '纪录片': '纪录片',
    '國外進口': '国外进口',
    '第一人稱攝影': '第一视角',
    '業餘': '业餘',
    '局部特寫': '局部特写',
    '獨立製作': '独立製作',
    'DMM獨家': 'DMM独家',
    '單體作品': '单体作品',
    '合集': '合集',
    '高清': '高清',
    '天堂TV': '天堂TV',
    'DVD多士爐': 'DVD多士炉',
    '堵嘴·喜劇': '堵嘴·喜剧',
    '幻想': '幻想',
    '性別轉型·女性化': '性别转型·女性化',
    '為智能手機推薦垂直視頻': '竖屏视频',
    '設置項目': '设置项目',
    '迷你係列': '迷你系列',
    '體驗懺悔': '体验懺悔',
    '黑暗系統': '黑暗系统',
    '騎在臉上': '颜面骑乘',
    '艾玛妮': '艾曼纽',
    '被遗弃': '放置',
    '擦洗口交': '清洁口交',
    '第一視角': '第一视角',
    '後入': '后入',
    '後入內射': '后入內射',
    '護士': '护士',
    '紀錄片': '纪录片',
    '嬌小': '娇小',
    '巨大陽具': '巨根',
    '巨大陰莖': '巨根',
    '劇情': '剧情',
    '老師': '教师',
    '戀物癖': '恋物癖',
    '旅館': '酒店',
    'S級女優': 'S级女优',
    '业餘': '业余',
    '亂交': '乱交',
    '体验懺悔': '体验忏悔',
    '做家務': '做家务',
    '側位內射': '侧位内射',
    '同性戀': '同性恋',
    '喪服': '丧服',
    '學校': '学校',
    '店員': '店员',
    '廚房': '厨房',
    '房間': '房间',
    '手銬': '手铐',
    '打底褲': '打底裤',
    '振動': '震动',
    '推薦作品': '推荐作品',
    '晚禮服': '晚礼服',
    '檢查': '检查',
    '沙發': '沙发',
    '治癒系': '治愈系',
    '淫亂真實': '淫乱真实',
    '淫蕩': '淫荡',
    '滥交': '滥交',
    '煩惱': '烦恼',
    '牛仔褲': '牛仔裤',
    '短褲': '短裤',
    '綁縛': '绑缚',
    '網襪': '网袜',
    '给女性观眾': '给女性观众',
    '職員': '职员',
    '肛門': '肛门',
    '蘿莉': '萝莉',
    '裸體襪子': '裸体袜子',
    '視訊小姐': '视讯小姐',
    '變態': '变态',
    '豐滿': '丰满',
    '購物': '购物',
    '轮姦': '轮奸',
    '連續內射': '连续内射',
    '連衣裙': '连衣裙',
    '遠程操作': '远程操作',
    '醫生': '医生',
    '長發': '长发',
    '門口': '门口',
    '陰道觀察': '阴道观察',
    '電動按摩器': '电动按摩棒',
    '電動陽具': '电动阳具',
    '不戴胸罩': '不穿胸罩',
    '主婦': '主妇',
    '內射': '内射',
    '不穿內褲': '不穿内裤',
    '偶像': '偶像艺人',
    '偷拍': '偷窥',
    '后入式': '后入式',
    '偷窺': '偷窥',
    '大胸': '巨乳',
    '大阴茎': '巨根',
    '女仆': '女佣',
    '女僕': '女佣',
    '强姦': '强奸',
    '搭訕': '搭讪',
    '最佳合集': '最佳女演员/合集',
    '服侍': '服务生',
    '模特': '模特',
    '泳衣': '泳装',
    '淫亂': '淫乱',
    '溫泉': '温泉',
    '满的高清': '高清',
    '騎乘位': '骑乘位',
    '首次': '首次亮相',
    '首次作品': '首次亮相',
    '颜射': '颜射',
    '第一人称摄影': '第一视角',
    '站立姿勢': '站立位',
    '白領': '白领',
    '独立製作': '独立影片',
    '無碼流出': '无码流出',
    '無碼破解': '无码流出',
    '潤滑劑': '含润滑液',
    '潤滑油': '含润滑液',
    '1080p': '1080p',
    '30多岁': '30多岁',
    '40多岁': '40多岁',
    '60fps': '60fps',
    '8K VR': '8K VR',
    '阿姨': '阿姨',
    '艾曼纽': '艾曼纽',
    '按摩按摩棒': '按摩棒',
    '蛮横娇羞': '傲娇',
    '伴侣': '伴侣',
    '薄马赛克': '薄码',
    '猥亵穿着': '暴露服装',
    '不穿内裤': '不穿内裤',
    '不穿胸罩': '不穿胸罩',
    '采访': '采访',
    '残忍画面': '残酷画面',
    '超乳足交': '超乳足交',
    '潮吹潮吹芑': '潮吹',
    '汽车性爱': '车内性爱',
    '出轨': '出轨',
    '白天出轨': '出轨',
    '通奸': '出轨',
    '处男': '处男',
    '打屁股': '打屁股',
    '打樁機': '打桩机',
    '单体作品': '单体作品',
    '主观视角': '第一视角',
    '第一视角': '第一视角',
    '女优按摩棒': '电动按摩棒',
    '电动按摩棒': '电动按摩棒',
    '短发': '短发',
    '恶作剧': '恶作剧',
    '粉丝答谢/探访': '粉丝答谢',
    'ぼっちゃり': '丰满/微胖',
    '健康与肥皂店': '风俗/泡泡浴',
    '风俗/泡泡浴': '风俗/泡泡浴',
    '卖淫': '风俗娘/性工作者',
    '風俗孃': '风俗娘/性工作者',
    '服务生': '服务生',
    '复刻版': '复刻版',
    'アクメ·オーガズム': '高潮',
    '高潮': '高潮',
    '高品质VR': '高品质VR',
    '高画质': '高清',
    '高个子': '高挑',
    '高個子': '高挑',
    '高挑': '高挑',
    '格斗家': '格斗',
    '寡妇': '寡妇',
    '灌肠': '灌肠',
    '国外进口': '海外进口',
    '汗流浃背': '汗流浃背',
    '和服/浴衣': '和服/浴衣',
    '和服•丧服': '和服/浴衣',
    '浴衣和服': '和服/浴衣',
    '黑暗系统': '黑暗系',
    '黑人演员': '黑人演员',
    '后入': '后入式',
    '及膝袜': '及膝袜',
    '妄想家庭': '家庭幻想',
    '女儿与继女': '女儿/继女',
    '假阳具': '假阳具',
    '女儿': '假阳具',
    '监禁': '监禁',
    '讲师': '讲师',
    '交换伴侣与换夫': '交换伴侣',
    '娇小的': '娇小',
    '角色扮演者': '角色扮演',
    '教学': '教学',
    '接待员': '接待员',
    'キス·接吻': '接吻',
    '接吻': '接吻',
    '介绍影片': '介绍/预告',
    '仅限流媒体播放': '仅限流媒体播放',
    '身体意识': '紧身裙',
    '紧身衣': '紧身衣',
    'ベスト': '精选合集',
    'ベスト·総集編': '精选合集',
    '精选合集': '精选合集',
    '酒店': '酒店',
    '局部特写': '局部特写',
    '巨根': '巨根',
    '多人大屁股': '巨臀',
    '巨大屁股': '巨臀',
    '戏剧': '剧情',
    '口內射精': '口爆',
    '变性者': '跨性别/变性者',
    '鸭嘴': '器械',
    '縛': '捆绑',
    '捆绑': '捆绑',
    '蜡烛': '蜡烛',
    '辣妹': '辣妹',
    '啦啦队长': '啦啦队',
    '爷爷': '爷爷',
    '立即性爱': '立即性爱',
    'パンスト·タイツ': '连裤袜/丝袜',
    '连裤袜': '连裤袜/丝袜',
    '恋爱': '恋爱/情侣',
    '情侣': '恋爱/情侣',
    '恋物癖': '恋物癖',
    '猎艳': '猎艳',
    '淋浴': '淋浴',
    '美胸': '凌辱',
    '羞耻/羞辱': '凌辱',
    '羞辱': '凌辱',
    '露乳': '露乳',
    '寝取り·寝取られ': '绿帽',
    '寝取り·寝取られ·NTR': '绿帽',
    '绿帽/戴绿帽': '绿帽',
    '乱伦': '乱伦',
    '轮奸': '轮奸',
    '裸体围裙': '裸体围裙',
    '猫耳女': '猫耳女',
    '美白': '美白',
    '美容院/按摩': '美容院/按摩',
    '美腿': '美腿',
    '美臀': '美臀',
    '迷你系列': '迷你系列',
    '秘书': '秘书',
    '苗条': '苗条',
    '名人': '名人',
    '模特儿': '模特',
    '母亲': '母亲',
    '男性喷水': '男性喷水',
    '内嵌字幕': '内嵌字幕',
    '内射': '内射',
    '内衣': '内衣',
    '女大学生': '女大学生',
    '女检察官': '女检察官',
    '女教师': '女教师',
    '女老板': '女老板',
    '女虐待狂': '女施虐',
    '女同性恋': '女同性恋',
    '给女性观众': '给女性观众',
    '女医生': '女医生',
    '女佣': '女佣',
    'AV女優': '女优',
    '女优精选合集': '女优精选合集',
    '女優ベスト·総集編': '女优精选合集',
    '女装人妖': '人妖/伪娘',
    'アイドル·芸能人': '偶像艺人',
    '偶像艺人': '偶像艺人',
    '泡泡袜': '泡泡袜',
    '小胸': '贫乳/微乳',
    '貧乳•微乳': '贫乳/微乳',
    '贫乳•微乳': '贫乳/微乳',
    '破解': '破解/流出',
    '其他恋物癖': '其他恋物癖',
    '骑乘位': '骑乘位',
    '轻熟女/姐姐': '轻熟女/姐姐',
    '干净': '清纯',
    '清纯': '清纯',
    '人妻·主婦': '人妻',
    '已婚妇女': '人妻',
    '日光浴': '日光浴',
    '柔软的身体': '柔软身体',
    '戏弄': '辱骂/责骂',
    '润滑液': '润滑油',
    '润肤露/油': '润滑油',
    '赛车女郎': '赛车女郎',
    '瘙痒': '瘙痒',
    '俱乐部活动与经理': '社团经理',
    '时问停止': '时间停止',
    '打手枪': '手交',
    '受虐狂': '受虐',
    '受虐狂女性': '受虐',
    '美丽成熟女性': '熟女',
    '数位马赛克': '数码马赛克',
    '双性人': '双性人',
    '业余': '素人',
    '素人作品': '素人',
    '射在外陰': '体外射精',
    '体验忏悔': '体验告白',
    '舔阴': '舔阴',
    '恋腿癖': '腿控',
    '后宮吞精': '吞精',
    '娃娃脸': '娃娃脸',
    '无码流出': '无码流出',
    '无毛': '无毛',
    '無套': '无套',
    '学校作品': '校园作品',
    '新娘': '新娘',
    '新娘、年轻妻子': '新娘',
    '性伴侶': '性伴侣',
    '性感': '性感',
    '性别转型•女性化': '性转/女性化',
    '羞耻': '羞耻',
    '堵嘴·喜刷処女': '需拆分：恶搞喜剧、处女',
    '学校泳装': '学校泳装',
    '顏面骑乘': '颜面骑乘',
    '颜面骑乘': '颜面骑乘',
    '眼镜': '眼镜',
    '药物': '药物',
    '野外•露出': '野外露出',
    '医院与诊所': '医院/诊所',
    '插入异物': '异物插入',
    '婊子': '淫乱',
    '淫乱': '淫乱',
    '花痴': '淫乱',
    '荡妇': '淫乱',
    '淫语': '淫语',
    '饮酒派对与集体约会': '饮酒派对/联谊',
    '饮尿': '饮尿',
    '硬核': '硬核',
    '泳装': '泳装',
    '浴室': '浴室',
    '与原创作品合作': '原作合作',
    '约会': '约会',
    '怀孕': '孕妇',
    '运动': '运动',
    '体育服': '运动服',
    '运动员': '运动员',
    '早泄': '早泄',
    '宅男宅女': '宅男宅女',
    '展场女孩': '展场女孩',
    '長時間': '长时间',
    '震动': '震动玩具',
    '下属与同事不忠': '职场出轨/不忠',
    '白领': '职场女性',
    '职业套装': '职场女性',
    '职场女性': '职场女性',
    '各种职业': '职业角色',
    '指法': '指法/手指插入',
    '制服剃毛': '制服剃毛',
    '子宫颈': '子宫颈',
    '手淫': '自慰',
    '自慰支持': '自慰辅助',
    'フェラ抜き': '口交',
    '女优ベスト': '女优精选合集',
    'ハードコア': '硬核',
    'アジアン': '亚洲',
    'パンティー': '内裤',
    '指フェラ': '吮指',
    '指入れ': '指法/手指插入',
    'ひざまずく': '跪姿',
    'ダブルフェラ': '双人口交',
    '乳首いじり': '乳首刺激',
    '完璧な尻': '美臀',
    '開いたマンコ': '阴部特写',
    'しゃぶり': '口交',
    'まんこ舐め': '舔阴',
    'アジアンフェラ': '亚洲口交',
    'オナニーソロ': '自慰',
    'キス': '接吻',
    '毛まみれ': '多毛',
    '集団プレイ': '群交',
    'ちんこ小さい': '小阴茎',
    'アジアンクリトリス': '亚洲阴蒂',
    'オフィス': '办公室',
    'セフレ': '性伴侣',
    'トリミング済み': '剃毛',
    'ビキニ': '比基尼',
    '天然おっぱい': '天然胸',
    '日本アップクローズ': '日本局部特写',
    '桃まんこ': '粉嫩阴部',
    'まんこ唇': '阴唇',
    'シャワー': '淋浴',
    'スーツ': '职业套装',
    'フェラ & 颜射': '口交/颜射',
    'マウスパイ': '口交',
    '尻舐め': '舔臀',
    '濡れたマンコ': '湿润阴部',
    '立ちセックス': '站立性爱',
    'いたずら': '恶作剧',
    'かわいい': '可爱',
    'まんこ': '阴部',
    'アジア美女': '亚洲美女',
    'ガーターベルト': '吊袜带',
    'ソファ': '沙发',
    'ネットストッキング': '网袜',
    'ピンクローター': '跳蛋',
    'ベッドセックス': '床上性爱',
    'ムチムチ': '丰满',
    'レジェンド女优': '传奇女优',
    'レジェンド女': '传奇女优',
    '乳首責め': '乳首刺激',
    '乳首吸引': '乳首吸吮',
    '女性の絶頂': '女性高潮',
    '完璧ボディ': '完美身材',
    '巨乳ママ': '巨乳妈妈',
    '熟女ママ': '熟女妈妈',
    '網タイツ': '网袜',
    '美しい': '美女',
    '茶髪ロング': '棕色长发',
    '金髪ママ': '金发妈妈',
    '颜座り': '颜面骑乘',
    '颜面ファック': '颜面口交',
    '饮み込み': '吞精',
    '黒髪ロング': '黑色长发',
    '10インチ以上': '10英寸以上',
    '4人プレイ': '4人性爱',
    'おしゃぶり': '口交',
    'おじさん': '大叔',
    'おっぱぶ': '胸部服务',
    'おもらし': '失禁',
    'お隣さん': '邻居',
    'かわいい日本人': '可爱日本人',
    'ご主人様': '主人',
    'のどフェラ': '深喉',
    'ぶっかけ5連発': '连续颜射',
    'まんぐり返し': '叉开双腿',
    'むっちり': '丰满',
    'アジアンアナル': '亚洲肛交',
    'アジアン中出し': '亚洲中出',
    'アジアン乱交': '亚洲乱交',
    'アジアン制服': '亚洲制服',
    'アジアン女子高生': '亚洲高中女生',
    'アジアン巨尻': '亚洲巨臀',
    'アジアン紧缚': '亚洲紧缚',
    'アジアン絶頂': '亚洲高潮',
    'アナルオナニー': '肛门自慰',
    'アナルディルド': '肛门假阳具',
    'アナルフィンガリング': '肛门指法',
    'アナル中出し': '肛内中出',
    'オムニバス': '合集',
    'カミソリ': '剃毛',
    'ガテン系': '蓝领',
    'ガムテープ': '胶带',
    'ザーメン': '精液',
    'ショートパンツ': '短裤',
    'ジム': '健身房',
    'ジーンズ': '牛仔裤',
    'スカート覗き': '裙底偷窥',
    'スケベイス': '情趣椅',
    'ストリッパー': '脱衣舞娘',
    'スリム': '苗条',
    'スレンダーボディー': '苗条身材',
    'セックスレクチャー': '性教学',
    'セックスレス': '无性婚姻',
    'ソファーセックス': '沙发性爱',
    'ソフトコア': '软核',
    'ソロ女子': '单人女性',
    'タートルネック': '高领毛衣',
    'テープ': '胶带',
    'デリヘル嬢': '上门风俗女',
    'トイレ': '厕所',
    '不倫・浮気': '出轨/不忠',
    '不倫·浮気': '出轨/不忠',
    'ネトラレ': '绿帽',
    'ハメ潮': '插入潮吹',
    'ハード部門': '硬核部门',
    'バスマット': '浴垫',
    'バランスボール': '瑜伽球',
    'パワハラ': '职场霸凌',
    'パンスト破り': '撕破丝袜',
    'パーティーゲーム': '派对游戏',
    'フィットネス': '健身',
    'フェラ & 射精': '口交/射精',
    'プール': '泳池',
    'ホットパンツ': '热裤',
    'ホテトル嬢': '酒店风俗女',
    'ポチャ': '丰满',
    'ポニーテール': '马尾',
    'マシーントレーニング': '器械训练',
    'マッサージ·リフレ': '美容院/按摩',
    'ムチ': '鞭子',
    'メイドコスプレ': '女佣角色扮演',
    'ロリ': '萝莉塔',
    'ワンピース': '连衣裙',
    '主観フェラ': '主观口交',
    '乳首舐め': '舔乳首',
    '元モデル': '前模特',
    '剃毛まんこ': '剃毛阴部',
    '口いっぱい': '满口',
    '営业マン': '业务员',
    '女王様の手コキ': '女王手交',
    '寝取られ': '绿帽',
    '巨大ディルド': '巨大假阳具',
    '広がり': '扩张',
    '広がる穴': '扩张穴',
    '引っ越し': '搬家',
    '彼の亲友': '男友的朋友',
    '机上セックス': '桌上性爱',
    '極太バイブ': '粗大按摩棒',
    '毛むくじゃら': '多毛',
    '濡れ': '湿润',
    '熟女アジアン': '亚洲熟女',
    '白パン': '白色内裤',
    '童貞を殺すセーター': '露肩毛衣',
    '背が高い': '高挑',
    '胸に発射': '射在胸部',
    '脱ぎ': '脱衣',
    '腹いせ': '报复',
    '茶髪ストレート': '棕色直发',
    '足フェチ': '足控',
    '連続中出し': '连续中出',
    '連続生中出し': '连续无套中出',
    '酔いつぶれ': '醉倒',
    '骑乘位中だし': '骑乘位中出',
    '骑乘位中出し': '骑乘位中出',
    '黒ストッキング': '黑色丝袜',
    '黒ニーソ': '黑色过膝袜',
    '黒髪ストレート': '黑色直发',
    '黒髪ストレートロング': '黑色长直发',
    'D罩杯': 'D罩杯',
    'DMM专属': 'DMM独家',
    'DMM独家': 'DMM独家',
    'E罩杯': 'E罩杯',
    'G罩杯': 'G罩杯',
    'MGS独家花絮': 'MGS独家花絮',
    'VR专用': 'VR',
}

# AV genre translations reviewed on 2026-07-14. Keep these overrides after the
# legacy map so the audited values are the effective values for duplicate keys.
MOVIE_GENRE_TRANSLATIONS.update(
    {
        '３P': '多P',
        '3P': '多P',
        '4P': '多P',
        '4人性爱': '多P',
        '10代': '10多岁',
        '18歳以上': '18岁以上',
        '白虎': '无毛',
        '白天出軌': '出轨',
        '绑缚': '捆绑',
        '卑猥': '猥亵',
        '背部骑乘位': '逆骑乘位',
        '背后插入': '后入式',
        '彼女': '女友',
        '筆': '笔',
        '変態': '变态',
        '不忠': '出轨',
        '挿入': '插入',
        '超VIP': 'VIP',
        '車': '汽车',
        '出轨/不忠': '出轨',
        '出軌': '出轨',
        '初登場': '出道',
        '处女作': '出道',
        '粗大按摩棒': '按摩棒',
        '打飞机': '自慰',
        '打飛機': '自慰',
        '打手槍': '自慰',
        '旦那': '丈夫',
        '第一人称视角': '第一视角',
        '电车痴汉': '性骚扰',
        '电动按摩棒': '按摩棒',
        '电动按摩器': '按摩棒',
        '電話': '电话',
        '電マ': '按摩棒',
        '丁字褲': '丁字裤',
        '独佔動画': '独家视频',
        '堵嘴·喜剧': '堵嘴',
        '多人': '多P',
        '多重提出する': '多次射精',
        '二度射精': '二次射精',
        '二穴挿入': '双穴同插',
        '二穴同插': '双穴同插',
        '二穴同入': '双穴同插',
        '訪问': '探访',
        '风呂': '浴室',
        '感谢祭': '粉丝答谢',
        '个室': '单间',
        '勾引': '诱惑',
        '龟甲捆绑': '捆绑',
        '喝酒': '饮酒',
        '喝口水': '吞精',
        '和风': '日式',
        '和服・喪服': '和服/丧服',
        '和服,喪服': '和服/丧服',
        '和服·丧服': '和服/丧服',
        '和室': '日式房间',
        '黑髮': '黑发',
        '后背位': '后入式',
        '换妻俱乐部': '交换伴侣',
        '混血美女': '混血',
        '精选合集': '合集',
        '精饮': '吞精',
        '精致身材': '完美身材',
        '巨乳｜': '巨乳',
        '巨乳爆乳': '巨乳',
        '开腿姿势': '开腿',
        '開脚': '开腿',
        '口塞': '堵嘴',
        '捆綁': '捆绑',
        '浪叫': '淫叫',
        '流汗': '汗流浃背',
        '六九式': '69',
        '輪姦中出し': '轮奸内射',
        '裸围裙': '裸体围裙',
        '媽媽': '母亲',
        '美人': '美女',
        '美少女｜': '美少女',
        '美少女電影': '美少女电影',
        '美マン': '美穴',
        '猛烈': '激烈',
        '面接': '面试',
        '面試': '面试',
        '魔女': '女巫',
        '目線': '视线',
        '南国度假地': '热带度假地',
        '女上司': '女上司',
        '女体料理': '女体盛',
        '女演员': '女优',
        '女优物': '女优',
        '女優按摩棒': '按摩棒',
        '女子校生': '高中女生',
        '女子校生制服': '高中女生制服',
        '漂亮屁股': '美臀',
        '贫乳': '贫乳/微乳',
        '平胸': '贫乳/微乳',
        '企劃物': '企划',
        '清楚系': '清纯',
        '人氣系列': '人气系列',
        '妊娠': '怀孕',
        '怀孕': '怀孕',
        '日本人妻': '人妻',
        '日焼け': '晒黑',
        '乳首': '乳头',
        '乳首刺激': '乳头刺激',
        '乳首吸吮': '乳头吸吮',
        '软体': '柔软身体',
        '润滑剂': '润滑油',
        '若妻·幼妻': '年轻妻子',
        '曬痕': '晒痕',
        '舌': '舌头',
        '舌頭': '舌头',
        '设置项目': '设定作品',
        '縄': '捆绑',
        '施虐受虐': 'SM',
        '實拍': '实拍',
        '瘦小身型': '娇小',
        '束缚': '捆绑',
        '私服': '便服',
        '送貨上門': '上门风俗女',
        '素人': '业余',
        '业余': '业余',
        '太太': '人妻',
        '套裝': '套装',
        '天然美乳': '美乳',
        '天然胸': '美乳',
        '外遇': '出轨',
        '完璧': '完美',
        '危険日': '易孕期',
        '圍裙': '围裙',
        '无套内射': '中出',
        '洗澡': '沐浴',
        '先輩教师': '前辈教师',
        '先生': '教师',
        '相談': '咨询',
        '小颜': '小脸',
        '携帯電話': '手机',
        '年輕妻子': '年轻妻子',
        '興奮': '兴奋',
        '虚拟现实': 'VR',
        '玄関': '门口',
        '靴下': '袜子',
        '学園': '学校',
        '雪白皮肤': '白皙肌肤',
        '眼镜娘': '眼镜',
        '業餘': '业余',
        '淫荡手淫': '自慰',
        '淫乱女': '淫乱',
        '幼妻': '年轻妻子',
        '渔网裤袜': '网袜',
        '与外国人玩': '外国人',
        '慾求不滿': '欲求不满',
        '約會': '约会',
        '孕ませ': '怀孕',
        '再會': '再会',
        '站立性交': '站立性爱',
        '真面目': '认真',
        '枕営业': '潜规则',
        '振动': '震动玩具',
        '主観视点': '第一视角',
        '自宅': '家中',
        '総集編': '合集',
        '足': '脚',
        '左右口交': '双人口交',
        '座布団': '坐垫',
        'M男': '受虐男',
        'M女': '受虐女性',
        'アクメ': '高潮',
        'アナルセックス（男の娘）': '伪娘肛交',
        'アナルバック': '后入肛交',
        'アナルマンコ同時挿入': '双穴同插',
        'イメージビデオ': '形象视频',
        'エプロン': '围裙',
        'お尻': '臀部',
        'ギャル系': '辣妹',
        'タレント': '艺人',
        'バス': '公交车',
        'ピチピチ': '紧身',
        'モデル系': '模特',
        '奥斯曼株式会社': '奥斯曼公司',
        '艺能人': '艺人',
        'AV OPEN 2015 素人部门': 'AV OPEN 2015 业余演员部门',
    }
)
