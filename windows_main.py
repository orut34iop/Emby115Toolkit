...
TKINTER_UNSUPPORTED_MESSAGE = (
    "windows_main.py 仅支持 Windows；macOS 请使用 PyQt5 版本：python macos_main.py；Linux 不再支持。"
)
...
class EmbyToolkit:
    def __init__(self, root):
        ...
        # 读取上次退出时的选项卡索引
        self.config = Config()
        last_tab_index = self.config.get('ui_state', 'selected_tab_index', 0)
    ...
    def init_tabs(self):
        """初始化所有标签页"""
        # 为每个标签页创建对应的类实例
        SymlinkExportTab(self.tabs["导出软链接"], self.log_dir)
        FolderToolsTab(self.tabs["文件夹操作"], self.log_dir)
    ...
    def on_tab_changed(self, event):
        ...
        self.config.set('ui_state', 'selected_tab_index', selected_tab_index)
        self.config.save()
