...
class TestFileMergerInit:
    ...
    def test_init_with_default_values(self, temp_dir):
        ...
        merger = FileMerger(metadata_folder=temp_dir, target_folder=temp_dir)

        assert merger.metadata_folder == temp_dir
    ...
    def test_init_with_custom_thread_count(self, temp_dir):
        ...
        merger = FileMerger(metadata_folder=temp_dir, target_folder=temp_dir, thread_count=8)
...
class TestFileMergerMatch:
    ...
    def test_match_identical_names(self, temp_dir, create_test_file_structure):
        ...
        # 扫描
        metadata_files = merger.scan(os.path.join(temp_dir, 'metadata'))
        target_files = merger.scan(os.path.join(temp_dir, 'target'))

        # 匹配
        matches = merger.match(metadata_files, target_files)
    ...
    def test_match_no_matches(self, temp_dir, create_test_file_structure):
        ...
        metadata_structure = {
            'metadata/Different.Name.nfo': 'nfo content',
        }
        create_test_file_structure(metadata_structure)
...
class TestFileMergerMerge:
    ...
    def test_merge_preserves_directory_structure(self, temp_dir, create_test_file_structure):
        ...
        metadata_structure = {
            'metadata/Season 1/episode01.nfo': 'ep1 nfo',
            'metadata/Season 1/episode01.jpg': 'ep1 poster',
        }
        create_test_file_structure(metadata_structure)
    ...
    def test_merge_skips_existing_files(self, temp_dir, create_test_file_structure):
        ...
        metadata_structure = {
            'metadata/Movie.2023.nfo': 'new nfo content',
        }
        create_test_file_structure(metadata_structure)
        ...
        # 源文件应该仍然存在（因为跳过了）
        assert os.path.exists(os.path.join(temp_dir, 'metadata', 'Movie.2023.nfo'))
...
class TestFileMergerCounters:
    ...
    def test_counters_after_run(self, temp_dir, create_test_file_structure):
        ...
        metadata_structure = {
            'metadata/Movie1.nfo': 'nfo1',
            'metadata/Movie2.nfo': 'nfo2',
            'metadata/Movie3.nfo': 'nfo3',
        }
        create_test_file_structure(metadata_structure)
