"""
services.symlink_checker 模块单元测试
"""
...
class TestSymlinkCheckerInit:
    ...
    def test_init_with_default_values(self, temp_dir):
        ...
        checker = SymlinkChecker(
            cloud_path=temp_dir, source_folder=temp_dir, target_folder=temp_dir, symlink_mode='symlink'
        )
    ...
    def test_init_strm_mode(self, temp_dir):
        ...
        checker = SymlinkChecker(
            cloud_path=temp_dir, source_folder=temp_dir, target_folder=temp_dir, symlink_mode='strm'
        )
...
class TestSymlinkCheckerGetSymlinkFiles:
    ...
    def test_get_symlink_files(self, temp_dir, create_test_file_structure):
        ...
        # 创建符号链接
        os.symlink(os.path.join(temp_dir, 'real.mp4'), os.path.join(temp_dir, 'link.mp4'))
...
class TestSymlinkCheckerCheckAndRemove:
    ...
    def test_skip_when_cloud_path_missing(self, temp_dir, create_test_file_structure):
        ...
        checker = SymlinkChecker(
            cloud_path='/nonexistent/cloud/path', source_folder=temp_dir, target_folder=temp_dir, symlink_mode='symlink'
        )
