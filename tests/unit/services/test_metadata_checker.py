"""
services.MedadaChecker 模块单元测试
"""

import os


class TestMetadataCheckerInit:
    """测试 MetadataChecker 初始化"""

    def test_init_with_default_values(self, temp_dir):
        """测试使用默认值初始化"""
        from services.metadata_checker import MetadataChecker

        checker = MetadataChecker(
            cloud_path=temp_dir, source_folder=temp_dir, target_folder=temp_dir, allowed_extensions=('.nfo', '.jpg')
        )
...
class TestMetadataCheckerGetMetadataFiles:
    ...
    def test_get_nfo_files(self, temp_dir, create_test_file_structure):
        ...
        checker = MetadataChecker(
            cloud_path=temp_dir, source_folder=temp_dir, target_folder=temp_dir, allowed_extensions=('.nfo',)
        )
...
class TestMetadataCheckerCheckAndRemove:
