...
class TestAutoUploaderCalculateTime:
    """测试 calculate_sync_interval 方法"""

    def test_calculate_sync_interval_with_integer(self):
        """测试纯数字字符串"""
        from services.auto_uploader import AutoUploader

        uploader = AutoUploader()
        result = uploader.calculate_sync_interval('3600')

        assert result == 3600

    def test_calculate_sync_interval_with_multiplication(self):
        """测试乘法表达式"""
        from services.auto_uploader import AutoUploader

        uploader = AutoUploader()
        result = uploader.calculate_sync_interval('2*3600')

        assert result == 7200

    def test_calculate_sync_interval_with_invalid_input(self):
        """测试无效输入返回默认值"""
        from services.auto_uploader import AutoUploader

        uploader = AutoUploader()
        result = uploader.calculate_sync_interval('not_a_number')

        assert result == 86400

    def test_calculate_sync_interval_with_complex_expression(self):
        """测试复杂乘法表达式"""
        from services.auto_uploader import AutoUploader

        uploader = AutoUploader()
        result = uploader.calculate_sync_interval('60*60*24')
...
class TestAutoUploaderRunOnce:
    ...
    def test_run_once_with_disabled_upload(self, temp_dir):
        ...
        # upload_enabled=False 时不应创建 MetadataCopier
        with patch('services.auto_uploader.MetadataCopier') as MockCopier:
            uploader.run_once()
            MockCopier.assert_not_called()
