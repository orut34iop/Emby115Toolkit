"""
services.symlink_creator 模块单元测试
"""
...
class TestSymlinkCreatorInit:
    ...
    def test_init_with_default_values(self, temp_dir):
        ...
        creator = SymlinkCreator(link_folders=[temp_dir], target_folder=temp_dir, symlink_mode='symlink')
...
class TestSymlinkCreatorScan:
    ...
    def test_scan_with_video_files(self, temp_dir, create_test_file_structure):
        ...
        # 软链接流程只处理配置的视频后缀，元数据由 MetadataCopier 负责
        assert len(result) == 2
    ...
    def test_scan_with_symlinks(self, temp_dir, create_test_file_structure):
        ...
        # 创建符号链接
        os.symlink(os.path.join(temp_dir, 'source', 'real_file.mp4'), os.path.join(temp_dir, 'source', 'link_file.mp4'))
...
class TestSymlinkCreatorOnlyTvshowNfo:
    ...
    def test_only_tvshow_nfo_true(self, temp_dir, create_test_file_structure):
        ...
        creator = SymlinkCreator(
            link_folders=[source_folder], target_folder=target_folder, symlink_mode='symlink', only_tvshow_nfo=True
        )
    ...
    def test_only_tvshow_nfo_false(self, temp_dir, create_test_file_structure):
        ...
        creator = SymlinkCreator(
            link_folders=[source_folder], target_folder=target_folder, symlink_mode='symlink', only_tvshow_nfo=False
        )
