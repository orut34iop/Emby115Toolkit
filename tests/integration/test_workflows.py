...
class TestSymlinkWorkflow:
    ...
    def test_strm_workflow(self, temp_dir, create_test_file_structure):
        ...
        creator = SymlinkCreator(link_folders=[source_folder], target_folder=target_folder, symlink_mode='strm')
...
class TestFileMergeWorkflow:
    ...
    def test_full_merge_workflow(self, temp_dir, create_test_file_structure):
        ...
        # 创建元数据文件夹
        metadata_structure = {
            'metadata/Movie.2023.nfo': 'nfo content',
            'metadata/Movie.2023.jpg': 'poster',
            'metadata/Movie.2023.srt': 'subtitle',
            'metadata/TVShow/tvshow.nfo': 'show nfo',
            'metadata/TVShow/Season 1/episode01.nfo': 'ep nfo',
        }
        create_test_file_structure(metadata_structure)
        ...
        merger = FileMerger(
            metadata_folder=os.path.join(temp_dir, 'metadata'),
            target_folder=os.path.join(temp_dir, 'target'),
            thread_count=1,
        )
        ...
        # 验证源文件被删除
        assert not os.path.exists(os.path.join(temp_dir, 'metadata', 'Movie.2023.nfo'))
    ...
    def test_merge_with_existing_files(self, temp_dir, create_test_file_structure):
        ...
        metadata_structure = {
            'metadata/Movie.2023.nfo': 'new nfo',
        }
        create_test_file_structure(metadata_structure)
        ...
        merger = FileMerger(
            metadata_folder=os.path.join(temp_dir, 'metadata'), target_folder=os.path.join(temp_dir, 'target')
        )
...
class TestTreeMirrorWorkflow:
    ...
    def test_full_tree_mirror_workflow(self, temp_dir, sample_tree_content):
        ...
        mirror = TreeMirror(tree_file=tree_file, export_folder=export_folder)
...
"""
        ...
        mirror = TreeMirror(tree_file=tree_file, export_folder=export_folder, fix_garbled_text=True)
...
class TestConfigIntegration:
    ...
    def test_config_with_symlink_creator(self, temp_dir, monkeypatch):
        ...
        config.set('symlink_export', 'thread_count', 8)
        config.set('symlink_export', 'only_tvshow_nfo', False)
        config.save()
        ...
        creator = SymlinkCreator(
            ...
            target_folder=config2.get('symlink_export', 'target_folder'),
            symlink_mode='symlink',
            thread_count=config2.get('symlink_export', 'thread_count'),
            only_tvshow_nfo=config2.get('symlink_export', 'only_tvshow_nfo'),
        )
    ...
    def test_config_with_emby_operator(self, temp_dir, monkeypatch):
        ...
        config.set('version_merge', 'api_key', 'my-api-key')
        config.save()
        ...
        # 使用配置创建 MediaServerClient
        operator = MediaServerClient(
            server_url=config2.get('version_merge', 'server_url'), api_key=config2.get('version_merge', 'api_key')
        )

        assert operator.server_url == 'http://emby-server:8096'
        assert operator.api_key == 'my-api-key'
...
class TestMultipleOperations:
    ...
    def test_symlink_and_merge_workflow(self, temp_dir, create_test_file_structure):
        ...
        # 创建元数据文件夹
        metadata_structure = {
            'metadata/Movie1.2023.nfo': 'nfo1',
            'metadata/Movie1.2023.jpg': 'poster1',
            'metadata/Movie2.2022.nfo': 'nfo2',
            'metadata/Movie2.2022.jpg': 'poster2',
        }
        create_test_file_structure(metadata_structure)
        ...
        creator = SymlinkCreator(link_folders=[source_folder], target_folder=target_folder, symlink_mode='symlink')
        creator.run()
        ...
        # 步骤 2: 合并元数据
        merger = FileMerger(metadata_folder=os.path.join(temp_dir, 'metadata'), target_folder=target_folder)
        merger.run()
...
"""
        ...
        # 步骤 1: 创建目录树镜像
        mirror = TreeMirror(tree_file=tree_file, export_folder=target_folder)
        mirror.run()
...
class TestErrorHandling:
    ...
    def test_symlink_creator_invalid_source(self, temp_dir):
        ...
        creator = SymlinkCreator(link_folders=['/nonexistent/path'], target_folder=temp_dir, symlink_mode='symlink')
    ...
    def test_file_merger_invalid_metadata_folder(self, temp_dir):
        """测试无效的文件合并元数据文件夹"""
        from services.file_merger import FileMerger

        # 应该抛出 FileNotFoundError
        with pytest.raises(FileNotFoundError):
            FileMerger(metadata_folder='/nonexistent/metadata', target_folder=temp_dir)

    def test_tree_mirror_invalid_file(self, temp_dir):
        """测试无效的树文件"""
        from services.tree_mirror import TreeMirror

        mirror = TreeMirror(tree_file='/nonexistent/tree.txt', export_folder=temp_dir)
...
class TestPerformance:
    ...
    def test_large_file_list_scan(self, temp_dir):
        ...
        creator = SymlinkCreator(link_folders=[source_folder], target_folder=temp_dir, symlink_mode='symlink')
    ...
    def test_multithreaded_symlink_creation(self, temp_dir):
        ...
        # 使用单线程模式（新版 run 方法）
        creator = SymlinkCreator(
            link_folders=[source_folder], target_folder=target_folder, symlink_mode='symlink', thread_count=1
        )
