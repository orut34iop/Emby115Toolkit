...
class TreeMirror:
    def __init__(self, tree_file: str, export_folder: str, fix_garbled_text=False, logger=None):
        """
        初始化 TreeMirror
        :param tree_file: 目录树文件路径
        :param export_folder: 导出文件夹路径
        :param fix_garbled_text: 修复乱码
        :param logger: 日志记录器
        """
        self.tree_file = tree_file
        self.export_folder = export_folder
        self.fix_garbled_text = fix_garbled_text
    ...
    def parse_tree(self) -> list:
        ...
        for line in lines:
            ...
            # 修复乱码
            if self.fix_garbled_text and '*' in name:
                name = name.replace('*', 's')
        ...
        for i, item in enumerate(raw_items):
            ...
            result.append({'name': name, 'is_dir': is_dir, 'depth': depth})
