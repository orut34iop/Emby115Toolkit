...
- TV TMDB search/details and episode details are implemented; TV scraping recognizes `{tmdb-123}` direct ID tags in the show folder or episode filenames before ordinary search, and if a year-filtered search returns no candidates it retries the same title without the year before falling through to LLM alias expansion. TV output uses `tvshow.nfo`, show poster/fanart/clearlogo, season posters, and per-episode NFO/thumb filenames following each episode video stem;
