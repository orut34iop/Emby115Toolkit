...
- CloudDrive2 task-based waiting is available through `upload_wait_strategy=clouddrive2` or `clouddrive2_or_fixed`. It polls CloudDrive2 gRPC `GetUploadFileList` for mounted-filesystem uploads (`operatorType=Mount`) matching the target path. Strict `clouddrive2` does not fall back; if no matching tasks are observed through the quiet window, Stage B continues because small metadata files may finish before polling can see them. If matching tasks are observed but confirmation times out, Stage B is skipped and the run reports `partial`. Explicit CloudDrive2 upload task errors still fail before moving videos. `clouddrive2_or_fixed` falls back to the fixed minute wait when confirmation times out or the API is unavailable;
