...
- CloudDrive2 task-based waiting is available through `upload_wait_strategy=clouddrive2` or `clouddrive2_or_fixed`. It polls CloudDrive2 gRPC `GetUploadFileList` for mounted-filesystem uploads (`operatorType=Mount`) matching the target path. Defaults are `clouddrive2.poll_interval_seconds=0.5` and `clouddrive2.settle_seconds=30` because CloudDrive2's upload list behaves like a short-lived active-task snapshot. Strict `clouddrive2` does not fall back; if no matching tasks are observed through the first quiet window, Stage B continues because small metadata files may finish before polling can see them. Once any matching task is observed, upload start is confirmed; completion is confirmed only when the task reaches a terminal success state or after a later continuous quiet window with no matching tasks. If confirmation times out, Stage B is skipped and the run reports `partial`. Explicit CloudDrive2 upload task errors still fail before moving videos. `clouddrive2_or_fixed` falls back to the fixed minute wait when confirmation times out or the API is unavailable;
...
- cloud card execution and CloudDrive2 probe execution use the same background run and SSE stack. During execution the cloud button changes to `取消执行` and can request cancellation for the active backend run;
