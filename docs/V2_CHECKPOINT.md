...
- TV TMDB search/details and episode details are implemented; TV scraping recognizes `{tmdb-123}` direct ID tags in the show folder or episode filenames before ordinary search, and if a year-filtered search returns no candidates it retries the same title without the year before falling through to LLM alias expansion. TV output uses `tvshow.nfo`, show poster/fanart/clearlogo/logo/thumb/banner artwork, best-effort clearart when the provider exposes it, season posters/thumbs including Specials artwork, and per-episode NFO/thumb filenames following each episode video stem;
...
- current implementation provides the Context Object contract, WebUI/CLI actions, config testing, config persistence APIs, movie and TV TMDB matching, movie/TV LLM alias retry for no-candidate cases, movie NFO, `tvshow.nfo`, episode NFO, poster/fanart/clearlogo/logo/thumb/banner downloading, best-effort clearart downloading, season poster/thumb downloading, and episode thumbnail downloading. LLM arbitration between multiple returned TMDB candidates and richer scoring are next-stage work.
...
- Before Stage A copies anything, each first-level media folder must pass a metadata completeness check. Movie folders require every video symlink in that folder tree to have a same-stem NFO. TV folders require root `tvshow.nfo`, root poster image (`poster.jpg` / `.jpeg` / `.png` / `.webp`), and every episode video symlink to have a same-stem NFO. If any requirement is missing, the whole first-level folder is not uploaded, no video move plan is created for it, and the report records `validate_media_folder` with `status=failed` and the missing paths;
- Stage A mirrors only complete scraped metadata directories and copies every non-symlink file to the target while excluding symlink files and symlink directories;
- Stage A does not move real videos;
- complete-folder video symlinks enter the Stage B real-video move plan only after the folder-level same-stem NFO checks pass. Normal eligible video symlinks are silently added to the move plan without per-file `skip_symlink_copy` rows, so the report highlights review-worthy skips and failures instead of normal flow bookkeeping;
