...
- Stage A mirrors valid scraped metadata directories and copies every non-symlink file to the target while excluding symlink files and symlink directories. TV first-level show directories must contain root `tvshow.nfo`; when `tvshow.nfo` is missing, the whole show directory is skipped, reported as `skip_tvshow_without_metadata`, and no upload or real-video move plan is created for that show;
- Stage A does not move real videos;
- symlink videos only enter the Stage B real-video move plan when the same directory contains a same-stem NFO file, for example `movie.mkv` requires `movie.nfo`. If that NFO is missing, the symlink is reported as skipped, no real-video move plan is created in real runs, and the run is marked `partial`. Normal eligible video symlinks are silently added to the move plan without per-file `skip_symlink_copy` rows, so the report highlights review-worthy skips and failures instead of normal flow bookkeeping;
